...
from basicsr.utils.checkpoint_manager import CheckpointManager
...
class SRModel(BaseModel):
    ...
    def __init__(self, opt):
        super().__init__(opt)
        self.net_g_weight_visual_config = opt['network_g'].pop('visual_config', None) or {}
        # define network
    ...
    def init_training_settings(self):
        ...
        # init gradient accumulater
        self._init_gradient_accumulation()

        # init weight log hook
        self._init_weight_hook()

        # CheckpointManager init
        max_checkpoints = self.opt['train'].get('max_checkpoints', -1)
        if max_checkpoints > 0:
            self.checkpoint_manager = CheckpointManager(max_keep=max_checkpoints)
            logger = get_root_logger()
            logger.info(f'[CheckpointManager] Keep last {max_checkpoints} checkpoints')
    ...
    def _init_weight_hook(self):
        weight_log_patterns = self.net_g_weight_visual_config.get('visualize_weight_layers', None)
        weight_log_freq = self.net_g_weight_visual_config.get('visualize_weight_freq', 500)
        net_name = self.net_g_weight_visual_config.get('name', 'net_g')
        if weight_log_patterns:
            self.setup_weight_gradient_hooks(
                target_net=self.net_g,
                net_name=net_name,
                layer_patterns=weight_log_patterns,
                log_freq=weight_log_freq
            )

    def optimize_parameters(self, current_iter):
        self._accumulation_step_begin()
    ...
    def save(self, epoch, current_iter):
        ...
        if self.checkpoint_manager is not None and current_iter != -1:
            pth_path = os.path.join(self.opt['path']['models'], f'net_g_{current_iter}.pth')
            state_path = os.path.join(self.opt['path']['training_states'], f'{current_iter}.state')
            self.checkpoint_manager.register(pth_path, state_path)