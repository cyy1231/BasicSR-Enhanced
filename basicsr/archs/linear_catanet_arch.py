...
class GlobalLinearAttention(nn.Module):
    """
    全局线性注意力，支持论文中提到的 ReLU / ELU+1 / Linear 核函数。
    对应论文公式 (2):
        o_i = φ(q_i)^T (Σ_j φ(k_j) v_j^T) / (φ(q_i)^T Σ_j φ(k_j))
    """
    def __init__(self, dim, heads, qk_dim, kernel='relu'):
        super().__init__()
        self.heads = heads
        self.qk_dim = qk_dim
    ...
    def apply_kernel(self, x):
        """论文 Sec.3.1 & Supp.B 讨论的三种核函数"""
        if self.kernel == 'relu':
            # ReLU: 丢弃负值，会损失负-负相关性信息
            return F.relu(x)
        elif self.kernel == 'elu':
            # ELU+1: 保留负值，但存在 +1 偏置导致注意力数值过大
            return F.elu(x) + 1.0
        elif self.kernel == 'linear':
            # 线性核（Identity）: φ(x)=x，无额外非线性
            return x
        else:
    ...
    def forward(self, x):
        ...
        # 线性注意力计算
        # 分子: φ(Q) @ (φ(K)^T @ V)
        kv = torch.matmul(k.transpose(-2, -1), v)       # (b, h, d_q, d_v)
        out = torch.matmul(q, kv)                        # (b, h, n, d_v)

        # 分母: φ(Q) @ sum(φ(K), dim=-2)
        k_sum = k.sum(dim=-2)                            # (b, h, d_q)
        denom = torch.matmul(q, k_sum.unsqueeze(-1)).squeeze(-1)  # (b, h, n)
        denom = denom.unsqueeze(-1) + self.eps             # (b, h, n, 1)
...
class LRSA(nn.Module):
    """
    结构: PreNorm(GlobalLinearAttention) + Residual → PreNorm(ConvFFN) + Residual
    """
    def __init__(self, dim, qk_dim, mlp_dim, heads=1, kernel='relu'):
        super().__init__()
        self.layer = nn.ModuleList([
            PreNorm(dim, GlobalLinearAttention(dim, heads, qk_dim, kernel)),
            PreNorm(dim, ConvFFN(dim, mlp_dim))
        ])

    def forward(self, x, ps=None):
        # ps 参数保留以兼容 CATANet 的调用接口，但不再使用
        _, _, h, w = x.shape
        x = rearrange(x, 'b c h w -> b (h w) c')

        attn, ff = self.layer
        x = attn(x) + x                      # 全局线性注意力 + 残差
        x = ff(x, x_size=(h, w)) + x       # ConvFFN + 残差
...
if __name__ == '__main__':
    ...
    model = LinearCATANet(upscale=3).cuda()
