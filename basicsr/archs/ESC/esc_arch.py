...
class ESC(nn.Module):
    def __init__(
        ...
        def _is_flex_available():
            """Flex Attention us available"""
            import platform
        ...
        if attn_type == 'Flex' and not _is_flex_available():
            logger = get_root_logger()
            logger.info(
                f'[ESC] Flex Attention is not available on Windows '
                f'(Triton missing). Falling back to SDPA.'
            )
            attn_type = 'SDPA'
